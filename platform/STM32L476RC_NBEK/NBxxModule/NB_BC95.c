...
static Bool cmd_next()
{
  ...
  if(g_nb_state.state == PROCESS_INIT)
  {
    ...
    case SUB_CGATT_QUERY:
      {
        ...
        g_at_cmd.max_timeout = 5000;  //相邻指令之间限制5s
        g_at_cmd.lmt_period = TRUE;
    ...
    case SUB_NSMI:
      {
        //设置coap 消息发送完通知
        cmd_param_init(&g_at_cmd,AT_NSMI,"0",CMD_SET);
      }
      break;
    case SUB_NNMI:
      {
...
uint8_t bc95_AsyncNotification(char* buf, uint16_t* len)
{
  ...
  if(position_addr_start = strstr(buf,"+CEREG"),position_addr_start)
  {
  ...
  else if(position_addr_start = strstr(buf,"+CSCON"),position_addr_start)
  {
    char* pColon = strchr(position_addr_start,':');
    if(pColon)
    {
  ...
  if(position_addr_start = strstr(buf,"+NSONMI"),position_addr_start)
  {
    //收到服务器端发来的UDP数据
    char* pColon = strchr(position_addr_start,':');
    if(pColon)
  ...
  else if(position_addr_start = strstr(buf,"+NNMI"),position_addr_start)
  {
    isAsync = addr_adjust(buf,position_addr_start,len);
    //isAsync =TRUE;
